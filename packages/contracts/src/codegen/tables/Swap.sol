...
library Swap {
  ...
  function setRate(IStore _store, uint256 rate) internal {
    bytes32[] memory _keyTuple = new bytes32[](0);
  ...
  function set(bool exists, uint256 rate, address index, uint256 margin, uint256 expiry) internal {
    bytes memory _data = encode(exists, rate, index, margin, expiry);
  ...
  function set(IStore _store, bool exists, uint256 rate, address index, uint256 margin, uint256 expiry) internal {
    bytes memory _data = encode(exists, rate, index, margin, expiry);
  ...
  function set(SwapData memory _table) internal {
    set(_table.exists, _table.rate, _table.index, _table.margin, _table.expiry);
  }

  /** Set the full data using the data struct (using the specified store) */
  function set(IStore _store, SwapData memory _table) internal {
    set(_store, _table.exists, _table.rate, _table.index, _table.margin, _table.expiry);
  }
  ...
  function encode(
    ...
    return abi.encodePacked(exists, rate, index, margin, expiry);
  }
